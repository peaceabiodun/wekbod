...
const SavedJobsTab = () => {
    return ( 
        <div>
            ...
            <div className='w-full my-5 grid grid-cols-2 mdLg:grid-cols-3'>

                {jobsData.map((item) => (
                <div key={item.id} className='flex flex-col border rounded-md p-2 my-3 mr-3 overflow-x-hidden whitespace-nowrap text-ellipsis'>
                    <div className='flex flex-row' >
                        <img src={JobFrame} alt="/" className='w-[60px]' />
                        <div className='flex flex-col justify-center' >
                            <p className='text-xs sm:text-sm font-semibold'>{item.title}</p>
                            <div className='flex gap-2 text-xs whitespace-nowrap text-ellipsis  '>
                                <p>{item.company}</p>
                                <p>{item.type}</p>
